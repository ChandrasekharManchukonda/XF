...
IF OBJECT_ID('dbo.Refresh_SFDC_Account_prc') IS NOT NULL
DROP PROCEDURE dbo.Refresh_SFDC_Account_prc

GO

CREATE PROCEDURE dbo.Refresh_SFDC_Account_prc
AS
BEGIN
	...
	DECLARE @Today DATETIME = GETDATE()
		...
		WHERE TableName = 'dim_SFDC_Account_tbl'
	...
	FROM dbo.SFDC_Accounts_tbl (NOLOCK)
	WHERE UpdDate > @MaxModifyDate

	IF EXISTS (SELECT top 1 1 FROM #XF_SFDC_Account)
	BEGIN
			INSERT INTO [dbo].[dim_SFDC_Account_tbl] (AccountID, AccountName)
			SELECT SFDCAccountID,AccountName
			FROM #XF_SFDC_Account X
			WHERE  NOT EXISTS (SELECT 1 FROM [dbo].[dim_SFDC_Account_tbl] A (NOLOCK) WHERE ISNULL(X.SFDCAccountID,'') = ISNULL(A.AccountId,''))
			...
			VALUES('dim_SFDC_Account_tbl',@Today,@MaxModifyDate)
	END

	DELETE FROM dbo.XF_Tracking_tbl WHERE TableName = 'dim_SFDC_Account_tbl' AND LastTrackingDate < DATEADD(YY,-1,@Today)
